...
%
% Notes:
%   - Split is done by patient, NOT by slice
%   - Binary segmentation: background vs tumor
% ==========================================
...
YTestCat  = masks_to_categorical(YTest, classNames);

%% -------------------------------
% 5. Optional augmentation
% -------------------------------
inputSize = size(XTrain,1:3);   % [H W C]
...
% Create augmentedImageDatastore for inputs
if useDataAugmentation
    dsXTrain = augmentedImageDatastore(inputSize(1:2), XTrain, ...
        'DataAugmentation', augmenter);
else
    dsXTrain = arrayDatastore(XTrain, 'IterationDimension', 4);
end

% For labels, do not use bilinear interpolation.
% We'll keep labels in a separate datastore and combine them.
...
% Building U-Net architechture
encoderDepth = 3;

lgraph = unetLayers(imageSize, numClasses, 'EncoderDepth', encoderDepth);

% Replace final pixel classification layer with class weights if needed
% Here we use simple unweighted classification first.
...
%% -------------------------------
% 9. Quick validation prediction preview
% -------------------------------
...
trainingConfig.useDataAugmentation = useDataAugmentation;
